...
par(mfrow = c(2, 2))
with(data_selected,plot(RealTime,Global_active_power,type="l",ylab="Golbal Active Power (killowatts)",
                        xlab="\n"))       # first graph
with(data_selected,plot(RealTime,Voltage,type="l",ylab="Voltage",
                        xlab="datetime"))   # Second Graph
...
legend("topright", lty="solid", col = c("black", "red","blue"), 
       legend = c("Sub_metering_1", "Sub_metering_2","Sub_metering_3"),cex=0.6)    # Third Graph

with(data_selected,plot(RealTime,Global_reactive_power,type="l",
                        xlab="datatime"))                                          #fouth graph
dev.off()